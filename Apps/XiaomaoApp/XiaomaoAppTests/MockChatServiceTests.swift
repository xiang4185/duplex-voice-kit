...
final class MockChatServiceTests: XCTestCase {
    ...
    func testSendReturnsAuthoritativeMessagesAndHistoryReadsThemBack() async throws {
        ...
        XCTAssertNil(sent.developerMessage)
        let developerResult = try XCTUnwrap(
            sent.participantResults.first(where: { $0.participant == .developer })
        )
        XCTAssertEqual(developerResult.status, .pending)
        XCTAssertNil(developerResult.message)
